...
| 6 | Volume / muted | `mediaController`, as a per-file record that outlives any single placement | default `{volume: 0, muted: true}` on first `acquire` | `VolumeControl` → `FileWindow` → `mediaController.setVolume(fileId, v)`. Never writes the DOM directly. Focus policy mutes unfocused windows **without** discarding the stored level | **RULED 2026-07-31: the record survives release.** `volume`/`muted` persist per file for the whole session across close and reopen; `currentTime` zeroes. This row previously said "dropped only on `release`", which contradicted §6 Q5 and made this row's own acceptance test unreachable after a close. Q5 is now the ruling; only `dispose` clears the record | `FileWindow` unit/dom test — "a window adopting a node already at 0.6 renders VOL at 060, not 000" |
...
**AMENDED 2026-07-31 — probe the ENCODES, never `raw/`.** An earlier draft of this section said to probe `raw/`. That is wrong: `process-media.sh` applies `-t 12` and `scale=-2:720`, so raw differs from what plays on both axes — `file03` is 17.792s raw versus 12.000s encoded, `file09` is 20.000s versus 12.000s, `file08` is 1080×1920 versus 406×720. Metadata must describe what the user actually sees. **Discover ids from `raw/`, probe `public/media/*_full.mp4`, and hard-error if a raw clip has no encode.** This section's own verified duration table and §4.4's "file08 406×720" were always encode numbers — the prose contradicted the data, and the data was right.
...
- **`_thumb` and `_full` must share an aspect ratio.** They currently do not: `file07`/`file08` thumb is 136×240 (0.5667) against full 406×720 (0.5639) — 0.493% apart, which sits right on this slice's own 0.5% tolerance and would put ~3px bars on an unfocused window, violating binding ruling 2. Generated metadata comes from `_full` only, so the thumb is the one that drifts. Fix in `process-media.sh` by deriving the thumb scale from the full's dimensions rather than scaling independently, then re-encode the thumbs. Add an assertion to the metadata guard test that every `_thumb` ratio matches its `_full` within 0.1%.
...
**5. ~~Whether volume persists per file across close and reopen.~~ RULED 2026-07-31: yes.** Held in the controller's per-file record for the session, surviving release; only `dispose` clears it. `currentTime` zeroes on release, since `load()` resets it anyway and the decode it pointed into is gone. §1 row 6 was amended to match — it previously said the record dropped on release, which contradicted this and made row 6's own acceptance test unreachable.
