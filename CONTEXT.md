...
- **Never overlap the glass strips.** The terminal is translucent, so anything painted under it shows through: a strip extended a pixel under the panel draws its own edge across the footer and down the left side. `--frame` is rounded to whole pixels instead, which closes the fractional gaps without putting anything beneath the glass.
