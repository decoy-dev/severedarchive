...
- The session dot pulses every 5s and drives a grainy wave across the title bar (`lib/pulseWave.ts`, `SessionPulse.tsx`). Each direction gets its **own reach**, because the dot is ~100px from the right edge and ~1200px from the left: with one reach the front cleared the right in the first instant and the whole pulse read as crossing leftward.
