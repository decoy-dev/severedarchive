...
for (const id of ids) {
  ...
  // The thumb's box is emitted too, purely so the "both encodes share an aspect
  // ratio" invariant is checkable in a unit test instead of by hand. A drift
  // here is bars on every unfocused window, which is exactly the class of thing
  // nobody notices until it ships.
  const thumb = resolve(MEDIA, `${id}_thumb.mp4`)
  if (!existsSync(thumb)) {
    console.error(`gen-media-meta: raw/${id}.mp4 has no public/media/${id}_thumb.mp4 — run scripts/process-media.sh`)
    process.exit(1)
  }
  const t = probe(thumb)
  thumbRows.push(`  ${id}: { width: ${t.width}, height: ${t.height} },`)
  console.log(`${id}  ${width}x${height}  ${durationSec}s  thumb ${t.width}x${t.height}`)
}
